...
class Factory(threading.Thread):
    ...
    def __init__(self, q, sem_to_produce, sem_to_sell, bar, id_no):
        super().__init__()
        self.q = q
        self.sem_to_produce = sem_to_produce
        self.sem_to_sell = sem_to_sell
        self.barrier = bar
        self.id_no = id_no
        self.car_count = 0
    ...
    def run(self):
        ...
        # TODO produce the cars, the send them to the dealerships
            self.sem_to_produce.acquire()
            self.q.put(Car())
            self.sem_to_sell.release()
            self.car_count += 1
...
class Dealer(threading.Thread):
    ...
    def __init__(self, q, sem_to_produce, sem_to_sell):
        super().__init__()
        self.q = q
        self.sem_to_produce = sem_to_produce
        self.sem_to_sell = sem_to_sell
    ...
    def run(self):
        while True:
            # TODO handle a car
            self.sem_to_sell.acquire()
            car = self.q.get()
            self.sem_to_produce.release()
            if not car:
                self.sem_to_produce.acquire()
                self.q.put(None)
                self.sem_to_sell.release()
...
def run_production(factory_count, dealer_count):
    ...
    to_produce = threading.Semaphore(MAX_QUEUE_SIZE)
    to_sell = threading.Semaphore(0)
    # TODO Create queue
    car_queue = Queue251()
    # TODO Create lock(s)
    ...
    factories = [Factory(car_queue, to_produce, to_sell, bar, id_no) for id_no in range(factory_count)]
    # TODO create your dealerships
    dealers = [Dealer(car_queue, to_produce, to_sell) for _ in range(dealer_count)]
    log.start_timer()
    ...
    for dealer in dealers:
        dealer.join()
    factory_stats = [*map(lambda x: x.car_count, factories)]
    dealer_stats = [*map(lambda x: x.car_count, dealers)]
